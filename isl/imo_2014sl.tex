...
\begin{enumerate}
	...
	Denote $m$ as the number of `close-call' regions. 
	From above, $m\ge n - \ell$. 
	For each of those finite regions, consider the contribution from each `blue' vertex (that is, the vertex is the intersection of two blue lines). 
	Let a region to have $r\ge 3$ sides; the number of blue vertices is $r-2$, 
	so each blue vertex contributes to $\frac{1}{r-2}\le 1$ of the blue vertices. 
	
	On the other hand, each blue vertex comes from intersection of two blue lines; 
	there are $\binom{\ell}{2}$ of them. 
	Considering the four regions the blue vertices correspond to, 
	we see that it can only contribute to at most 4 `close-call' regions. 
	This gives $4\binom{\ell}{2}\ge m\ge n-\ell$. 
	Rearranging yields $\ell\ge \sqrt{n/2}$. 
	
	For a sharper bound, we first show that each blue vertex can only correspond to at most two close-call regions that are triangles. 
	Suppose $ABC$ is a finite region of a triangle, with $A$ being blue and $B, C$ not. 
	Then the region on the $AB$ side (that's opposite $C$) cannot be a triangle; neither can the region on the $AC$ side (that's opposite $B$). 
	Hence the only other possible triangle is the one opposite $A$. 
	
	Next, if these $ABC$ and $ADE$ are close-call triangles determined by blue vertex $A$, then $BC$ and $DE$ aren't blue. 
	Both $BC$ and $DE$ correspond to the two regions beside the two triangles, 
	which have two non-blue lines and hence not close-call. 
	It then follows that the contribution of this blue vertex is at most 2. 
	
	Finally, if a vertex $A$ corresponds to a close-call triangle $ABC$, we'll show that the other three regions cannot be all close-call quadrilaterals. 
	If the two regions beside this triangle are close-call quadrilaterals, then there will be two additional blue lines, 
	one from each side region,
	that extends through the region opposite our triangle. 
	This region now has 4 blue lines, so if it were to be a close-call, it needs to have at least 5 vertices. 
	Thus the contribution of this vertex is at most $1 + \frac 12 + \frac 12 + \frac 13=\frac 73$. 
	
	Summarizing above, the contribution of each blue vertex is at most $\frac 73$, 
	so from $\frac 73\binom{\ell}{2}\ge m\ge n-\ell$ gives $\ell\ge \sqrt{6n/7}$. 
