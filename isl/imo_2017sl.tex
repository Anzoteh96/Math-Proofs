...
\usepackage{amsmath, amssymb, amsthm, fullpage, mathrsfs, bm, pgf, tikz}
\usepackage{mathrsfs, prettyref}
...
\item [\textbf{A2}] Let $q$ be a real number. Gugu has a napkin with ten distinct real numbers written on it, and he writes the following three lines of real numbers on the blackboard:
    ...
    Now in view of \prettyref{eq:a6_2}, we consider 
    ...
    \begin{equation}\label{eq:a6_5}
    	f(x + y) - f(x) = f(y) + 1
    \end{equation}
    Indeed, fix $d = 2(x + y)$ into \prettyref{eq:a6_4} gives 
    ...
    Finally, plugging $n = 2$ into \prettyref{eq:a6_2} we get $f(2d) = 2f(d) + 1$, 
    and therefore 
    ...
    \]
    If $x_0=0$ then $f(x_0)=f(0)=-1=f(x_1)$, and by \prettyref{eq:a6_1} this means $x_1=0$ too, so $x_1=0$. 
...
\begin{enumerate}
    ...
    \textbf{Solution.} 
    For each number $cm$, consider the function $f:\bbN\to\bbN$ such that $f(m)$ is the largest divisor of $m$ that's relatively prime to 10 
    (e.g. the first few values of $f$ are $1, 1, 3, 1, 1, 3, 7, 1, 9, 1, \cdots$). 
    Notice that a rational number is short iff it's in the form of $\frac pq$ where $\gcd(p, q)=1$ and $q$ has no prime divisor other than 2 or 5. 
    Thus, $\frac{10^t-1}{cm}$ short if and only if $f(cm)\mid 10^t - 1$. 
    
    Now, $t$ being $m$-tastic means that $\ord_{f(cm)}(10)=t$ for some $c\le 2017$. 
    Therefore, for each $m$, the $m$-tastic numbers are 
    \begin{equation}
    	\{\ord_{f(cm)}: c=1, 2, \cdots, 2017\}
    \end{equation}
    Given that $f(m)=f(cm)$ whenever $c$'s prime divisors cannot be anything other than 2 or 5, 
    i.e. $f(cm) = f(f(c)m)$. 
    This means $S(m)\le |\{f(c): c=1, \cdots, 2017\}|=|c=1, \cdots, 2017: c\equiv 1, 3, 7, 9\pmod{10}|$, 
