...
\begin{enumerate}
    ...
    \item [\textbf{C5}]
        ...
        \begin{proof}
        	Notice that $A$ is a DAG, so when $|B|=0$, $\sum_{i\in A} d_i^2$ is maximized as per \prettyref{lmm:sq_deg}. 
            ...
            \begin{flalign*}
            	...
            	+ 4w|A_1|(|A_1| - D') + \frac{D'^3-D'}{3} - \frac{w^3 - w}{3}
            	\\&=\frac{D'^3 + 3D'w^2 - 3D'^2w - w^3 - (D'-w)}{3}
            	\\&=\frac{D^3-D}{3}
            \end{flalign*}
            as desired. 
        ...
        $\sum_{a\in A} e_a + \sum_{b\in B} e_b= 0$, and \prettyref{lmm:sq_diff}. 
