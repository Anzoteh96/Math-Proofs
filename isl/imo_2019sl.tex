...
\usepackage{amsmath, amssymb, fullpage, amsthm, mathrsfs, bm, pgf, tikz}
\usepackage{mathrsfs}
...
\begin{enumerate}
	...
	%Now for each vertex $v\neq v_0$, denote $c(v)$ as the unique vertex such that 
	%the subgraph induced by the $n-1$ queries has edge $v\to c(v)$
	%Given this claim, we may now perform topological sorting using $<$ (i.e. if $a < b, b < c$ then $a < c$), such that $v < c(v)$. 
	%Notice that $v < v_0$ for all vertices $v \neq v_0$. 
...
\begin{enumerate}
	...
	\definecolor{uuuuuu}{rgb}{0.26666666666666666,0.26666666666666666,0.26666666666666666}
	\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm]
	\clip(-10.971232482661602,-4.5877742343874925) rectangle (1.8020795587887677,5.762301979195165);
	...
	We first start with the following: 
	\begin{equation}
		\frac{IZ_1}{Z_1D}=\frac{MI}{MD}\cdot \frac{\sin\angle XMA_1}{\sin\angle FMX}
		=\frac{MI}{MD}\cdot \frac{XA}{XF}\frac{\sin\angle XAM}{\sin\angle XFM}
	\end{equation}
	the second equality due to $M, A, A_1$ collinear and 
	\begin{equation}
		\frac{MA\sin\angle XMA}{MF\sin\angle FMX}
		=\frac{|\triangle XAM|}{|\triangle XFM|}
		=\frac{MA\cdot XA\sin\angle XAM}{MF\cdot FX\sin\angle XFM}
	\end{equation}
	Similarly, 
	\begin{equation}
		\frac{IZ_2}{Z_2D}
		=\frac{NI}{ND}\cdot \frac{YA}{YE}\frac{\sin\angle YAN}{\sin\angle YEN}
	\end{equation}
	First, both $\angle YAN=\angle XAM=90^{\circ}$ since $\ell$ is perpendicular to $A_1I$. 
	In addition, by the properties of cyclic quadrilateral we have 
	\begin{equation}
		\angle XFM = 180^{\circ} - \angle PFD = 180^{\circ} - \angle PED = \angle YEN
	\end{equation}
	Hence $\frac{\sin\angle XAM}{\sin\angle XFM} = \frac{\sin\angle YAN}{\sin\angle YEN}$. 
	
	Next, let's study the ratios $\frac{XA}{YA}$ and $\frac{XF}{YE}$. 
	We see that $EF\parallel XY$, so $\frac{XF}{YE} = \frac{PF}{PE}$. 
	In addition, since $A, P, R$ collinear, $PFRE$ is harmonic so 
	$\frac{PF}{PE} = \frac{RF}{RE}$. 
	This also implies that in the context of both triangles $PEF$ and $PYX$, $PA$ is the symmedian line from line $P$. 
	It then follows that $\frac{XA}{YA}=(\frac{PF}{PE})^2 = (\frac{RF}{RE})^2$
	
	Finally, by sine rule, 
	\begin{equation}
		\frac{MI}{MD}
		=\frac{\sin\angle MDI}{\sin\angle MID}
		=\frac{\sin\angle FDI}{\sin\angle MID}
	\end{equation}
	(recall that sines of supplementary angles are equal), 
	and also 
	\begin{equation}
		\frac{NI}{ND}
		=\frac{\sin\angle NDI}{\sin\angle NID}
		=\frac{\sin\angle EDI}{\sin\angle NID}
	\end{equation}
	Since $DR\perp EF$, $DR, DI$ are cevian conjugates of each other; 
	with some angle chasing we have $\angle RFE = \angle RDE = \angle IDF$ and similarly 
	$\angle REF = \angle IDE$. Also $\angle MID = \angle NID$ as $M, N, I$ collinear. 
	
	We're now ready to put things together. Here, 
	\begin{equation}
		\frac{MI}{MD}\cdot \frac{XA}{YA}
		=\frac{\sin\angle FDI}{\sin\angle MID}\left(\frac{RF}{RE}\right)^2
		=\frac{\sin\angle RFE}{\sin\angle MID}\left(\frac{\sin\angle REF}{\sin\angle RFE}\right)^2
		=\frac{\sin^2\angle REF}{\sin\angle MID\sin\angle RFE}
	\end{equation}
	while 
	\begin{equation}
		\frac{NI}{ND}\cdot \frac{XF}{YE}
		=\frac{\sin\angle NDI}{\sin\angle NID}\frac{RF}{RE}
		=\frac{\sin\angle REF}{\sin\angle NID}\frac{\sin\angle REF}{\sin\angle RFE}
		=\frac{\sin^2\angle REF}{\sin\angle MID\sin\angle RFE}
	\end{equation}
	So the ratios above are indeed equal. 
	Together with $\frac{\sin\angle XAM}{\sin\angle XFM} = \frac{\sin\angle YAN}{\sin\angle YEN}$, 
	we just have to rearrange terms to get 
	\begin{equation}
		\frac{IZ_1}{Z_1D}
		=\frac{MI}{MD}\cdot \frac{XA}{XF}\frac{\sin\angle XAM}{\sin\angle XFM}
		=\frac{NI}{ND}\cdot \frac{YA}{YE}\frac{\sin\angle YAN}{\sin\angle YEN}
